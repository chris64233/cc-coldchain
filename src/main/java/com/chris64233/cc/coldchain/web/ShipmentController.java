...
public class ShipmentController {
    ...
    public ShipmentController(ColdChainService coldChainService) {
    ...
    public ShipmentResponse createShipment(@Valid @RequestBody CreateShipmentRequest request) {
        return ShipmentResponse.from(coldChainService.createShipment(
                request.waybillNo(),
                request.tempMin(),
                request.tempMax(),
                request.maxBreachSeconds(),
                request.initialHolder(),
                request.departureTime()));
    }

    @GetMapping("/{waybillNo}")
    public ShipmentResponse getShipment(@PathVariable String waybillNo) {
        return ShipmentResponse.from(coldChainService.getShipment(waybillNo));
    }
    ...
    public TemperatureEventResponse recordTemperatureEvent(@PathVariable String waybillNo,
        ...
        return TemperatureEventResponse.from(coldChainService.recordTemperatureEvent(
                waybillNo, request.eventId(), request.sampledAt(), request.temperature()));
    }
    ...
    public List<TemperatureEventResponse> listTemperatureEvents(@PathVariable String waybillNo) {
        return coldChainService.listTemperatureEvents(waybillNo).stream()
                .map(TemperatureEventResponse::from)
                .toList();
    }
    ...
    public HandoverResponse initiateHandover(@PathVariable String waybillNo,
        ...
        return HandoverResponse.from(coldChainService.initiateHandover(
                waybillNo, request.fromHolder(), request.toHolder()));
    }
    ...
    public HandoverResponse confirmHandover(@PathVariable String waybillNo,
        ...
        return HandoverResponse.from(coldChainService.confirmHandover(
                waybillNo, handoverId, request.confirmedBy()));
    }

    @GetMapping("/{waybillNo}/handovers")
    public List<HandoverResponse> listHandovers(@PathVariable String waybillNo) {
        return coldChainService.listHandovers(waybillNo).stream()
